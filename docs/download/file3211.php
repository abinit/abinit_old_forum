...
!arrays
 ...
 real(dp),allocatable :: fofgin_down(:,:), fofr_down(:,:,:,:)    ! --by gbliu---
...
!&       use_gpu_cuda=Dtset%use_gpu_cuda)
 ...
 &       use_gpu_cuda=Dtset%use_gpu_cuda)     ! ---by gbliu---
!      deallocate(fofgout)
...
! --------------------uncommented by gbliu-----------------------
       if (Dtset%nspinor==2) then
       allocate(fofgin_down(2,npw_k))       
       allocate(fofr_down(2,n4,n5,n6))       
...
!g     ABI_ALLOCATE(ur,(2,n1*n2*n3))
       ABI_ALLOCATE(ur,(2,n1*n2*n3*ndat))   ! ---by gbliu---
       ABI_ALLOCATE(ur_ae_onsite,(2,n1*n2*n3))
       ABI_ALLOCATE(ur_ps_onsite,(2,n1*n2*n3))
!g     ABI_ALLOCATE(ur_pw,(2,n1*n2*n3))
       ABI_ALLOCATE(ur_pw,(2,n1*n2*n3*ndat))   !---by gbliu----
...
!g             ur(1,ifftsph) = ur(1,ifftsph) + re_p * (phj-tphj)
!g             ur(2,ifftsph) = ur(2,ifftsph) + im_p * (phj-tphj)
               ur(1,ifftsph+(ispinor-1)*nfftot) = ur(1,ifftsph+(ispinor-1)*nfftot) + re_p * (phj-tphj)     !--by gbliu--
               ur(2,ifftsph+(ispinor-1)*nfftot) = ur(2,ifftsph+(ispinor-1)*nfftot) + im_p * (phj-tphj)     !--by gbliu--
...
!      * Remove the phase e^{ikr}, we store u(r).
       ...
       enddo
       write(*,'(A,2I5,F22.16)', advance='no') 'ikpt,iband, norm (G,PSWF)=',ikpt,iband,norm
       !------------------------------------------

       norm=zero
       do ifft=1,nfftot*Dtset%nspinor     !--changed by gbliu--
         norm = norm + ur(1,ifft)**2+ur(2,ifft)**2
       end do
       norm=norm/nfftot 

       norm_rerr = MAX((ABS(norm-one))*100,norm_rerr)
       write(std_out,*)"norm (R,AEWF)= ",norm    !--changed by gbliu--
